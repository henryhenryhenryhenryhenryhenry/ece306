...
volatile unsigned int ADC_Right_Detector_raw = 0;
volatile unsigned int ADC_Left_Detector_raw = 0;
...
unsigned int R_det_max = RESET_STATE;
unsigned int R_det_min = RESET_STATE;
unsigned int L_det_max = RESET_STATE;
unsigned int L_det_min = RESET_STATE;
...
__interrupt void ADC12_ISR(void){
  ...
  case ADC12IV__ADC12IFG2:            //vector 16: ADC12MEM2 interrupt
    ...
    ADC_Thumb_raw = ADC12MEM0;
    ADC_Right_Detector_raw = ADC12MEM1;  //big # means black, small # means white
    ADC_Left_Detector_raw = ADC12MEM2;   //big # means black, small # means white
    break;
...
void display_values(void){
  HEXtoBCD(ADC_Left_Detector);//600 - 2600
  strcpy(display_line[LINE_1ST], adc_val_str);
  update_string(display_line[LINE_1ST], LINE_1ST);
  
  HEXtoBCD(ADC_Right_Detector);// 200 - 2400
  strcpy(display_line[LINE_2ND], adc_val_str);
  update_string(display_line[LINE_2ND], LINE_2ND);
  
  HEXtoBCD(total_time_running * 5 / 2);
  strcpy(display_line[LINE_3RD], adc_val_str);
  update_string(display_line[LINE_3RD], LINE_3RD);
  
  /*
  if(ADC_Left_Detector < 1100 && ADC_Right_Detector < 500){//white
    strcpy(display_line[LINE_0TH], "on white  ");
    update_string(display_line[LINE_0TH], LINE_0TH);
  }else if(ADC_Left_Detector > 2000 && ADC_Right_Detector > 2000){//black
    strcpy(display_line[LINE_0TH], "on black  ");
    update_string(display_line[LINE_0TH], LINE_0TH);
  }
  */
}